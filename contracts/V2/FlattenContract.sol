...
contract Collection is ERC721URIStorage {
    ...
    function getUserNFTs(address owner_) public view returns (UriTokenId[] memory) {
        uint256 size = 0;
        uint256 current = _tokenIds.current();
        for (uint256 i = 0; i < current; i++) {
            if (ownerOf(i) == owner_) { size += 1; }
        }
        uint256 idx = 0;
        UriTokenId[] memory tokenUris = new UriTokenId[](size);
        for (uint256 i = 0; i < current; i++) {
            if (ownerOf(i) == owner_) { 
                tokenUris[idx] = UriTokenId({
                    uri: tokenURI(i),
                    id: i
                });
                idx += 1;
...
abstract contract CollectionHolder {
    ...
    function inCollectionArray(address _address) public view returns(bool) {
        bool isIn = false;
    ...
    function createCollection(
        ...
        require(
            findCollection(msg.sender, collectionName) == false
        );
    ...
    function collectionToJson (Collection collection, uint256 likes) internal view returns (CollectionJSON memory) {
        return CollectionJSON({
                collectionName: collection.name(),
                collectionAddress: address(collection),
                collectionImage: collection
                    ._collectionHolderImage(),
                collectionDescription: collection._description(),
                collectionLikesCount: collection
                    .getAllLiked()
                    .length,
                collectionOwner: collection.owner(),
                collectionNftLikes: likes,
                categories: collection.getCategory()
            });
    }
    ...
    {
        CollectionJSON[] memory myCollections = new CollectionJSON[](_collectionIds.current());
        for (uint256 i = 0; i < _collectionIds.current(); i++) {
            Collection collection = collectionsArray[i];
            uint256 totalNftLikes = 0;
            for (
                uint256 j = 0;
                j < collection.currentTokenId();
                j++
            ) {
                totalNftLikes += collection
                    .getNFTsAllLiked(j)
                    .length;
            }
            CollectionJSON memory json = collectionToJson(collection, totalNftLikes);
            myCollections[i] = json;
        }
...
contract NFTMarketplaceV2 is UserHolder, CollectionHolder, MarketItemHolder {
    ...
    function createMarketToken(
        ...
        payable
    {
        require(inCollectionArray(collectionAddress));
        require(price > 0);
